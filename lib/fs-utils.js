 ...
 *      Backfill hands us every non-licensed swimmer, and a large share of them
 *      (retired officials, duplicate profiles, never-competed registrations)
 *      have an empty grid. Persisting those produced ~1k zero-race files that
 *      bloated the index and, worse, marked those ids as "already on disk" so
 *      computeBackfillCandidates never revisited them. Skipping the write keeps
 *      them as candidates AND keeps the index to swimmers who actually raced.
 *
...
export function writeSwimmerFile(swimmer, swimmersDir) {
  ...
  // adds no data — skip it so it stays a backfill candidate and the index stays
  // clean. We check both candidate paths; if neither exists yet, there is
  // nothing to preserve and nothing worth creating.
  ...
  fs.writeFileSync(target, JSON.stringify(swimmer, null, 2), "utf-8");
  return target;
}
...
/**
 * Read the entry currently stored at the <club>/<name>.json path that the
 * given swimmer WOULD be written to. Backfill uses this to detect when a
 * candidate id shares a name+club with an already-populated file (a duplicate
 * medley.no profile), so it can avoid handing processSwimmer a code path that
 * clobbers real data with an empty grid.
 *
 * @returns {object|null} the on-disk entry, or null if nothing is there.
 */
export function readSwimmerFileFor({ name, club, swimmerId }, swimmersDir) {
  const clubDir = safeFilename(club || "ukjent");
  const namePart = safeFilename(name || swimmerId);
  return readEntryIfExists(path.join(swimmersDir, clubDir, `${namePart}.json`));
}

/* ─── Checked-empty ledger ────────────────────────────────────────── */

/**
 * The checked-empty ledger records swimmerIds that were successfully checked
 * during backfill and confirmed to have 0 races. These swimmers deliberately
 * get NO swimmer file (writeSwimmerFile guard #3), which keeps the index and
 * total count restricted to swimmers who actually raced. But "no file" also
 * meant computeBackfillCandidates re-queued them every run, so the same
 * confirmed-empty swimmers were re-discovered and re-scraped forever.
 *
 * This ledger is the missing third state: "checked, confirmed empty". It is
 * subtracted from the candidate pool so an empty swimmer is checked exactly
 * once, but it never feeds rebuildIndex, so it never inflates totalSwimmers.
 *
 * The verdict is permanent by design (these are overwhelmingly retired
 * officials, never-competed registrations, and duplicate profiles). To
 * re-check them — e.g. if an old swimmer competes again — delete this file or
 * run with FORCE_RECHECK_EMPTY=1, which makes loadCheckedEmpty return an empty
 * set for the run.
 *
 * Shape: { generatedAt, swimmers: [{ id, name, club, checkedAt }] }
 */

/**
 * Load the checked-empty ledger as a Map<id, entry>. Returns an empty map when
 * the file is missing/unreadable or when force is true (manual re-check reset).
 */
export function loadCheckedEmpty(ledgerFile, { force = false } = {}) {
  const map = new Map();
  if (force) return map;
  try {
    if (!fs.existsSync(ledgerFile)) return map;
    ...
    if (data && Array.isArray(data.swimmers)) {
      for (const entry of data.swimmers) {
        if (entry && entry.id != null) map.set(String(entry.id), entry);
      }
    }
  } catch {
    /* unreadable ledger — treat as empty, it will be rewritten */
  }
  return map;
}

/**
 * Append swimmerIds to the checked-empty ledger and persist it. Existing
 * entries are preserved (their original checkedAt is kept); genuinely new ids
 * are added with the current timestamp. Writing is idempotent — re-adding an
 * id already present is a no-op for that id.
 *
 * @param {string} ledgerFile
 * @param {Map<string,object>} existingMap - the map returned by loadCheckedEmpty
 * @param {Array<{id:string,name?:string,club?:string}>} newlyEmpty
 * @param {string} dataDir - ensured to exist before writing
 * @returns {number} count of ids newly added this call
 */
export function saveCheckedEmpty(ledgerFile, existingMap, newlyEmpty, dataDir) {
  const now = new Date().toISOString();
  let added = 0;
  for (const sw of newlyEmpty) {
    const id = String(sw.id);
    if (existingMap.has(id)) continue;
    existingMap.set(id, {
      id,
      name: sw.name ?? sw.text ?? null,
      club: sw.club ?? null,
      checkedAt: now,
    });
    added++;
  ...
  const swimmers = [...existingMap.values()].sort((a, b) =>
  ...
  try {
    ...
    fs.writeFileSync(
      ledgerFile,
      JSON.stringify({ generatedAt: now, swimmers }, null, 2),
      "utf-8",
    );
  } catch (err) {
    console.warn(`⚠ Could not write checked-empty ledger: ${err.message}`);
  }
  return added;
}
