...
/**
 * Tokenize a name into words, normalizing whitespace and lowercasing.
...
/**
 * Check if two swimmer names match, handling format variations.
 * Tries: exact match, normalized match, and various reversed formats.
 * 
 * Handles multi-word names like "Isak Bakk Kværnø" vs "Bakk Kværnø; Isak"
 * by trying multiple surname splits (last 1, 2, or 3 words).
 */
function namesMatch(pageShowsName, expectedFromCombo) {
  const page = pageShowsName.trim();
  const expected = expectedFromCombo.trim();

  // Exact match
  if (page === expected) return true;

  // Normalized match (remove extra whitespace and semicolons)
  const norm = (s) => s.replace(/;\s*/g, " ").replace(/\s+/g, " ").toLowerCase();
  if (norm(page) === norm(expected)) return true;

  const pageTokens = tokenizeName(page);
  const expectedTokens = tokenizeName(expected);

  // If expected is "Last; First Middle..." format (has semicolon)
  if (expectedFromCombo.includes(";")) {
    const [surname, ...firstNames] = expectedTokens;
    
    // Try: "First Middle... Last" vs "Last; First Middle..."
    // Check if page starts with first names and ends with surname
    if (pageTokens.length >= 2) {
      const pageFirst = pageTokens.slice(0, -1);
      const pageLast = pageTokens[pageTokens.length - 1];
      
      if (
        pageLast === surname &&
        pageFirst.length === firstNames.length &&
        pageFirst.every((t, i) => t === firstNames[i])
      ) {
        return true;
      }
    }
    
    // Try partial matches: page might have fewer middle names
    // Check if "First [Middle...] Last" in page matches expected
    if (pageTokens.length >= 2) {
      const pageLast = pageTokens[pageTokens.length - 1];
      if (pageLast === surname) {
        // Last word matches — good enough if first words include all expected first names
        const pageFirstWords = pageTokens.slice(0, -1);
        if (pageFirstWords.join(" ").includes(firstNames.join(" "))) {
          return true;
        }
      }
    }
  ...
  // If expected is "First Middle... Last" format (no semicolon in expected)
  // Try splitting page on various surname positions
  if (!expectedFromCombo.includes(";")) {
    // Try: page is "Last; First Middle..." and expected is "First Middle... Last"
    if (pageShowsName.includes(";")) {
      const [pageSurname, ...pageFirstNames] = pageTokens;
      
      // Check if reversing gives us the expected order
      if (expectedTokens.length >= 2) {
        const expectedLast = expectedTokens[expectedTokens.length - 1];
        const expectedFirst = expectedTokens.slice(0, -1);
        
        if (
          expectedLast === pageSurname &&
          expectedFirst.length === pageFirstNames.length &&
          expectedFirst.every((t, i) => t === pageFirstNames[i])
        ) {
          return true;
        }
      }
    ...
    // Try multi-word surname splits: assume last 1, 2, or 3 words could be surname
    for (let surnameWords = 1; surnameWords <= Math.min(3, pageTokens.length - 1); surnameWords++) {
      const pageFirstWords = pageTokens.slice(0, -surnameWords);
      const pageSurname = pageTokens.slice(-surnameWords).join(" ");
      
      // Check if this split matches "First Middle... Last" order
      const reconstructed = [...pageFirstWords, pageSurname].join(" ");
      if (reconstructed.toLowerCase() === expected.toLowerCase()) {
        return true;
      }
      
      // Also check reversed: "Last; First Middle..."
      const reversedExpected = `${pageSurname}; ${pageFirstWords.join(" ")}`;
      if (reversedExpected.toLowerCase() === expected.toLowerCase()) {
        return true;
      }
    }
  }
