...
import { flattenRaces, writeSwimmerFile, saveSkipUntil } from "./fs-utils.js";
...
export async function processSwimmer(page, sw, selIdx, ctx) {
  ...
  // D-rows ("deldistanser") are kept — they represent intermediate split
  // attempts within longer races. Their splits are truncated to the first
  // 2 segment times in extractSplits, and a `partOf` field is added to
  // reference the parent race discipline. The index race count excludes
  // them (r.D !== "D" in rebuildIndex), so they don't inflate totals.

  // Capture gender NOW while the grid still shows this swimmer's data.
  // The split extraction below may trigger a page reload (aliveness check),
  // after which the page shows the DEFAULT swimmer — so we must capture
  // gender before that point.
  const capturedGender =
    prevData?.gender || (await fetchGender(page).catch(() => null)) || null;
  ...
  if (ctx.mode !== "splits") {
    ...
    // Preserve previously scraped split data for eligible races that are
    // still in the grid but were not re-fetched (collect mode does not
    // call extractSplits, so splits from previous runs must be merged).
    const prevSplits = ctx.existingSwimmers.get(sw.id);
    if (prevSplits && prevSplits.timestamp) {
      const savedRaces = flattenRaces(prevSplits);
      for (const r of races) {
  ...
  if (existing && existing.timestamp) {
    ...
    if (savedRaces.length === races.length) {
      ...
      if (missingSplits.length === 0) {
        // Backfill D column from grid onto existing data.
        // Previously scraped swimmers don't have D (D/F flag), and this
        // path writes existing data back unchanged — so we add it here.
        const dLookup = new Map();
        for (const r of races) {
          if (r.D) {
            dLookup.set(`${r.Distanse}|${r.Dato}|${r.Tid}`, r.D);
          }
        }
        if (dLookup.size > 0) {
          for (const d of existing.disciplines) {
            for (const sr of d.races) {
              if (sr.D === undefined) {
                const key = `${d.distanse}|${sr.Dato}|${sr.Tid}`;
                const val = dLookup.get(key);
                if (val) sr.D = val;
              }
            }
          }
        ...
        if (!existing.gender && capturedGender) {
          existing.gender = capturedGender;
        }
        console.log(
  ...
  const info = await getSwimmerInfo(page);
  const swimmerName = info.name || sw.text;
  const gender = existing?.gender || capturedGender;
