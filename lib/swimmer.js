...
/**
 * Check if two swimmer names refer to the same person, handling format
 * variations like "First Middle Last" vs "Last; First Middle".
 *
 * Uses a word-set comparison: both names are normalized (semicolons removed,
 * lowercased, split into words, sorted alphabetically) and compared. This
 * handles all reorderings and separator differences.
 *
 * The only false-positive risk — two different people with the exact same
 * set of name words — is vanishingly unlikely in practice.
 */
function namesMatch(pageShowsName, expectedFromCombo) {
  const normalize = (s) =>
    s
      .toLowerCase()
      .replace(/[;]+/g, " ")
      ...
      .sort()
      .join(" ");
  return normalize(pageShowsName) === normalize(expectedFromCombo);
}
 ...
 */
export async function processSwimmer(
  page,
  sw,
  selIdx,
  ctx,
  ...
  // Prefer selecting by stable id (drift-proof and filter-proof). Fall back to
  // the positional index only if the id isn't in the combo's client data store
  // (e.g. it wasn't loaded). selIdx may be a filtered-subset index after a
  // name-filter lookup, so id-based selection is the reliable primary path.
  const selectedById = sw.id
    ? await selectSwimmerById(page, sw.id).catch(() => false)
    : false;
  if (!selectedById) {
  ...
  const gridReady = await pollFor(
    ...
    () => {
      try {
        ...
        return (
          ...
          // Empty grid renders a <tr class="dxgvEmptyDataRow_PlasticBlue …">
          // (theme-suffixed, and the class lives on the tr, not a td) — match
          // on the class substring so both the suffix and the element are
          // handled.
          table.querySelector('[class*="dxgvEmptyDataRow"]') !== null
        );
      } catch {
        return false;
      }
    ...
    {
      interval: 300,
      // Backfill grids show FULL history and can take longer to render than
      // the small incremental window; allow callers to widen the poll budget.
      timeout: ctx.gridPollTimeoutMs ?? 5_000,
    },
  );
  ...
  if (!gridReady) {
    if (existingEntry) {
      ...
      console.log(
        color(
          C.yellow,
          `  ⚠ ${sw.text} — grid never loaded (existing data preserved)`,
        ),
      );
      return { saved: false, totalRaces: 0, totalNewRaces: 0 };
    }
    if (ctx.skipIfGridNeverLoaded) {
      // Backfill: a brand-new swimmer whose full-history grid failed to load
      // must NOT be written as 0 races — that would permanently lose their
      // entire career. Leave them as a candidate for a future run instead.
      console.log(
        color(
          C.yellow,
          `  ⚠ Grid never loaded — ${sw.text} (skipped, will retry next run)`,
        ),
      );
      return {
        saved: false,
        totalRaces: 0,
        totalNewRaces: 0,
        gridNeverLoaded: true,
      };
    }
    // Brand-new swimmer whose grid never loaded. If a file already exists we
    // refresh it; otherwise writeSwimmerFile declines to create a zero-race
    // file (returns null) and we leave them a candidate for a future run.
    console.log(
      color(C.yellow, `  ⚠ Grid never loaded — ${sw.text} (0 races)`),
    );
    ...
    const written = writeSwimmerFile(entry, ctx.SWIMMERS_DIR);
    return {
      saved: written !== null,
      totalRaces: 0,
      totalNewRaces: 0,
      gridNeverLoaded: written === null,
    };
  }
  ...
  if (!races || races.length === 0) {
    ...
    console.log(
      color(C.yellow, `  ⚠ No data — ${sw.text} (0 races)`),
    );
    ...
    const written = writeSwimmerFile(entry, ctx.SWIMMERS_DIR);
    return { saved: written !== null, totalRaces: 0, totalNewRaces: 0 };
  }

  /* ── Capture gender while the grid still shows this swimmer ──── */
  // Gender doesn't change, so reuse a previously-determined value and skip
  // the extra resultat.aspx fetches. Only fetch when we don't have one yet.
  const gender =
    existingEntry?.gender ?? (await fetchGender(page).catch(() => null));
   ...
   *
   * Handles format variations like "First Last" vs "Last; First"
   * and multi-word names with multiple middle names.
   */
  const idCheck = await getSwimmerInfo(page).catch(() => null);
  if (idCheck?.name && sw.text) {
    if (!namesMatch(idCheck.name, sw.text)) {
      console.log(
        color(
          C.yellow,
          `  ⚠ Swimmer identity mismatch — page shows "${idCheck.name}", expected "${sw.text}" (will retry)`,
        ),
      );
      return {
        saved: false,
  ...
  // A grid row without a parseable PID cannot be deduped or split-fetched,
  // so it is dropped. This should be rare; warn loudly if the Tid-link
  // format ever changes so we don't silently lose races.
  const pidless = races.filter((r) => !r.pid);
  if (pidless.length > 0) {
    console.warn(
      color(
        C.yellow,
        `  ⚠ ${sw.text} — ${pidless.length} grid row(s) had no PID and were dropped (${pidless
          .map((r) => `${r.Distanse} ${r.Dato}`)
          .slice(0, 5)
          .join(", ")})`,
      ),
    );
  ...
  if (newRaces.length === 0 && existingEntry) {
    console.log(
      color(
        C.dim,
        `  — ${sw.text} — all ${races.length} races already indexed`,
      ),
    );
    return { saved: false, totalRaces: 0, totalNewRaces: 0 };
  }
  ...
  /* ── Write to disk ───────────────────────────────────────────── */
  const written = writeSwimmerFile(finalEntry, ctx.SWIMMERS_DIR);
  const totalRaces = races.length;
  const totalNewRaces = newRaces.length;

  // writeSwimmerFile returns null when it declines to create a brand-new file
  // for a zero-race swimmer. Report it as not-saved so the caller leaves the
  // swimmer uncached (no lastChecked stamp, no processedIds entry) and re-checks
  // them next run, picking up their races once they ever compete.
  if (written === null) {
    console.log(
      color(
        C.dim,
        `  — ${sw.text} — 0 races, not persisted`,
      ),
    );
    return { saved: false, totalRaces: 0, totalNewRaces: 0 };
  }
