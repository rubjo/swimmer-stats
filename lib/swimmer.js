...
export async function processSwimmer(page, sw, selIdx, ctx) {
  ...
  // Filter out deldistanser ("D" rows) — they are intermediate split-distance
  // entries for longer races and should not be counted as separate races.
  for (let ri = races.length - 1; ri >= 0; ri--) {
    if (races[ri].D === "D") races.splice(ri, 1);
  }
