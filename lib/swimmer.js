...
export async function processSwimmer(
  ...
  // for a zero-race swimmer. Report it as not-saved so the caller leaves it a
  // backfill candidate (no lastChecked stamp, no processedIds entry) and can
  // retry later if the swimmer ever competes.
  if (written === null) {
    console.log(
      color(
        C.dim,
