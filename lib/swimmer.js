 ...
 * Tries: exact match, normalized match, and reversed format (Last; First vs First Last).
 */
function namesMatch(pageShowsName, expectedFromCombo) {
  ...
  if (expectedParts.length === 2) {
    // Expected is "Last; First" format — try "First Last"
    const reversed = `${expectedParts[1].trim()} ${expectedParts[0].trim()}`;
    if (page.toLowerCase() === reversed.toLowerCase()) return true;
  }
  
  // Page might be "First Last" format — try to match against "Last; First"
  const pageParts = page.split(/\s+/);
  if (pageParts.length >= 2) {
    // Assume first part(s) are first names, last part is surname
    const pageFirstName = pageParts.slice(0, -1).join(" ");
    const pageSurname = pageParts[pageParts.length - 1];
    const reversed = `${pageSurname}; ${pageFirstName}`;
    if (reversed.toLowerCase() === expected.toLowerCase()) return true;
  }
