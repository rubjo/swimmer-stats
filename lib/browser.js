...
export async function extractSplits(
  ...
  for (let vi = 0; vi < csvRaces.length; vi++) {
    ...
    try {
      ...
      // Check if the click actually started a callback
      const afterClick = await page.evaluate(
        (idx) => ({
          inCallback: grdRanking ? grdRanking.InCallback() : "?",
          detailExists: grdRanking?.GetDetailRow(idx) !== null,
        }),
        vi,
      );
      if (!afterClick.inCallback && !afterClick.detailExists) {
        log(
          `    ⚠ click on row ${vi}: no callback triggered,` +
            ` no detail row appeared`,
        );
      }
