...
export async function findSwimmerIdx(page, name, maxScrolls = 500) {
  ...
  // If still 0 after 30s, close + reopen once more
  if (total === 0) {
    await page.evaluate(() => {
      try {
        cmbUtover.HideDropDown();
  ...
  // Helper: normalise a name for fuzzy comparison — lowercases, replaces
  // semicolons with spaces, collapses multiple whitespace into one.
  const norm = (s) =>
    s.replace(/;\s*/g, " ").replace(/\s+/g, " ").trim().toLowerCase();

  // Helper: scan loaded items for the target name, returning { found, idx }
      ...
      (targetName, normStr) => {
        ...
        try {
          ...
          for (let i = 0; i < total; i++) {
            ...
            // Normalised (handles whitespace/semicolon variations)
            if (n(itemText) === normStr) return { found: true, idx: i };
            ...
            if (targetParts.length === 2) {
              ...
              if (n(itemText) === n(reversed)) return { found: true, idx: i };
            } else {
              const parts = itemText.split(/;\s*/);
              if (parts.length === 2) {
                const reversed = `${parts[1].trim()} ${parts[0].trim()}`;
                if (n(reversed) === normStr) return { found: true, idx: i };
              }
            }

            // Substring match (catches stray characters in item text)
            if (n(itemText).includes(normStr) || normStr.includes(n(itemText)))
  ...
  // Helper: scroll the combo dropdown, waiting for items to load
  async function scrollAndWait() {
    return page.evaluate(async () => {
      try {
        ...
        if (d.scrollTop <= prevScroll) {
          // Can't scroll further — wait for items to arrive
          for (let p = 0; p < 30; p++) {
            await new Promise((r) => setTimeout(r, 500));
            const cur = cmbUtover.GetItemCount();
            if (cur > prevCount) return { exhausted: false, count: cur };
          }
          return { exhausted: true, count: prevCount };
        }

        // Scroll position changed — poll for the item count to grow
        ...
        // Fall through — give it one more second and return whatever
  ...
  for (let s = 0; s < maxScrolls; s++) {
    ...
    const { exhausted } = await scrollAndWait();
    if (exhausted) break;
  }

  // ── Phase B: close + reopen (resets scroll position, loads prefix) ──
  // Mirrors loadAllComboItems Phase B: items from Phase A are cached;
  // reopening shows prefix items that may not have been loaded yet.
  await page.evaluate(() => {
    try {
      cmbUtover.HideDropDown();
    } catch {}
  ...
  // Scan again after reopen (prefix items may now be accessible)
  {
    const scan = await scanLoaded();
    if (scan.found) {
      await page.evaluate(() => {
  ...
  // ── Phase C: scroll to bottom again ────────────────────────────────
  for (let s = 0; s < maxScrolls; s++) {
    const scan = await scanLoaded();
    if (scan.found) {
      await page.evaluate(() => {
