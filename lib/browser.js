...
export async function findSwimmerIdx(page, name, opts = {}) {
  ...
  // The filter term: prefer the surname (token before ";"), which is what the
  // "Last; First" combo entries start with, so the server returns a tight set.
  const filterTerm = (target.split(/;/)[0] || target).trim();
...
async function findSwimmerIdxByFilter(page, target, targetNorm, filterTerm, id = null) {
  ...
  // OWN debounced server callback — WebForm_DoCallback(...) — which is invoked
  // by the control's internal key handler. Dispatching bare DOM key events on
  // cmbUtover_I does NOT reach that handler, so the previous approach never
  // filtered: ShowDropDown() just rendered the default first 100-item batch
  // (the alphabetical head of the list), which is why every non-A surname
  // "returned 100 candidates" and then failed to match.
  //
  // The supported trigger is the widget's own PerformCallback(), which fires
  // the same server callback user-typing uses; it reads the current edit text
  // as the filter term. We set the input text first so the callback filters on
  // our surname, then read the (small) settled result set.
  const applied = await page.evaluate((term) => {
    try {
      cmbUtover.HideDropDown();
    } catch {}
    try {
      cmbUtover.SetText(term);
      // Mirror the term into the raw <input> so the control's own filter
      // parameter (read from the edit area on callback) matches `term`.
      const input =
        (typeof cmbUtover.GetInputElement === "function" &&
          cmbUtover.GetInputElement()) ||
        document.getElementById("cmbUtover_I");
      if (input) input.value = term;

      // Primary trigger: the widget's own server callback. For an
      // ASPxClientComboBox in filter mode, the incremental-filter callback
      // reads the current edit text — which we've just set — so call
      // PerformCallback() and let it pick up the term from the input. Passing
      // the term explicitly as the callback arg is also accepted by most
      // versions; we omit it to match exactly what user-typing does.
      if (typeof cmbUtover.PerformCallback === "function") {
        cmbUtover.PerformCallback();
      }
      cmbUtover.ShowDropDown();
      return true;
  ...
  if (!applied) return null; // trigger never ran → caller may try scroll scan

  // Wait for the filter callback to finish AND items to arrive. A genuinely
  // filtered surname set is tiny (typically 1–10 rows), so this settles in a
  // few seconds — no scrolling required.
  let count = 0;
  for (let w = 0; w < 40; w++) {
    await sleep(300);
