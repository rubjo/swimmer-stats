  ...
  for (let i = 0; i < eligible.length; i += CONCURRENCY) {
    ...
    for (const r of results) {
      if (r.splits !== undefined) {
        let splits = r.splits || [];
        // If this is a D-row, trim the returned full split list to the
        // number of 50m segments corresponding to the D-row distance.
        if (csvRaces[r.vi]?.D === "D") {
          const distMatch = String(csvRaces[r.vi].Distanse || "").match(/^(\d+)m/);
          if (distMatch) {
            const dist = parseInt(distMatch[1], 10);
            const expectedSegments = Math.max(1, Math.round(dist / 50));
            if (splits.length > expectedSegments) {
              splits = splits.slice(0, expectedSegments);
            }
          }
        }
        csvRaces[r.vi].splits = splits;
        if (splits.length > 0) splitsFound++;
      }
