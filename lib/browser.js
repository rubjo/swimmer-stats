...
export async function navigateAndFilter(page, baseUrl) {
  ...
  // Trigger a callback so the server processes the filter checkbox change.
  // SetChecked(true) alone does NOT send the value to the server in this DevExpress version.
  await page.evaluate(() => {
    return new Promise((resolve) => {
      try {
        ctl00_ctl00_Content_MainContent_chkKunLisensiert.SetChecked(true);
        // Fire the checkbox's callback via __doPostBack so the server
        // picks up the filter before we start loading swimmers.
        __doPostBack("ctl00$ctl00$Content$MainContent$chkKunLisensiert", "");
      } catch {}
      // Resolve immediately; we catch the reload below.
      resolve();
    });
  });
  // Wait for the postback to finish reloading the page
  ...
  // Re-apply the filters that don't survive a postback
  await page.evaluate(() => {
    dtFraDato.SetDate(new Date(2000, 0, 1));
    chkKunForste.SetChecked(false);
    chkIkkeVisDeldistaner.SetChecked(true);
...
/**
 * Expand every grid row, extract the detail info table fields
 * (Stevne, etc.) and split times from the sub-grid,
 * then collapse the row. Attaches all data to the matching race.
 */
export async function extractSplits(
  ...
  for (let vi = 0; vi < csvRaces.length; vi++) {
    ...
    try {
      ...
      await sleep(expandDelay + jitter(jitterRange));

      // Extract fields from the detail info table
      const fields = await page.evaluate((idx) => {
        const detailRow = grdRanking?.GetDetailRow(idx);
        if (!detailRow) return {};
        const result = {};
        // All field rows have an id starting with the detail control name
        const rows = detailRow.querySelectorAll("table.auto-style2 tr[id]");
        for (const row of rows) {
          const labelEl = row.querySelector('span[style*="font-weight:bold"]');
          const valueEl = row.querySelector("td:last-child > span");
          if (labelEl && valueEl) {
      ...
      if (fields.Stevne) race.Stevne = fields.Stevne;

      // Extract splits if the sub-grid exists — just segment times
      ...
      log(
        `    ⚠ detail extraction failed for row ${vi}: ${err.message?.slice(0, 80)}`,
      );
    }
  }
}
