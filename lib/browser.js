  ...
  for (let vi = 0; vi < csvRaces.length; vi++) {
    ...
    try {
      ...
      if (!detailReady) {
        ...
        if (detailExists) {
          // Debug: dump detail row details
          const diag = await page.evaluate((idx) => {
            const dr = grdRanking?.GetDetailRow(idx);
            if (!dr) return { html: "(no detail row)" };
            const html = dr.innerHTML;
            const hasMellom = html.includes("grdMellomtider");
            // Find all table IDs in the detail row
            const tables = Array.from(dr.querySelectorAll("table[id]")).map(
              (t) => t.id,
            );
            return {
              hasMellomtider: hasMellom,
              tableIds: tables,
          ...
          log(
            `    - row ${vi}: hasMellomtider=${diag.hasMellomtider} tables=${JSON.stringify(diag.tableIds)} htmlLen=${diag.htmlLength}`,
          );
          log(`    - row ${vi}: detail HTML:\n${diag.html}`);
          race.splits = [];
