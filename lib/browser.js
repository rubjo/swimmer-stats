...
export async function pollFor(
  page,
  condition,
  { interval = 200, timeout = 15_000, evalTimeout = 15_000 } = {},
  ...args
) {
  const evalOpts = { timeout: evalTimeout };
  const start = Date.now();
  while (Date.now() - start < timeout) {
    try {
      const result = await page.evaluate(condition, evalOpts, ...args);
...
export async function extractSplits(
  page,
  csvRaces,
  { log = () => {}, expandDelay = 10_000 } = {},
) {
  const evalOpts = { timeout: 15_000 };
  ...
  for (let vi = 0; vi < csvRaces.length; vi++) {
    ...
    try {
      ...
      // If CDP timed out the page is likely wedged — bail on the rest
      if (
        err.message?.includes("timed out") ||
        err.message?.includes("Timeout")
      ) {
        log(`    ⚠ bailing out of split extraction for this swimmer`);
        break;
      }
    }
  }
