...
export async function extractSplits(
  ...
  page
    .evaluate(() => ({
      visibleCount: grdRanking ? grdRanking.GetVisibleRowsCount() : -1,
    ...
    .then((d) =>
      log(
        `grid: ${d.visibleCount} visible rows, ${d.dataRows} data rows in DOM`,
      ),
    )
    .catch(() => {});
        ...
        if (detailExists) {
          ...
          log(
            `    ⚠ split table not ready for row ${vi}` +
              ` (grid: ${diag.visibleCount} visible, ${diag.dataRows} data rows,` +
