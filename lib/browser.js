 ...
 * data store (cmbUtover.GetItem(idx) !== null). Opens the dropdown once
 * and scrolls incrementally (one viewport per iteration) to trigger
 * sequential batch loading from the server.
 *
 * After a page reload, only the first batch (~100 items) is loaded.
 * GetItemCount() returns the loaded count at that point, not the server
 * total — so we cannot rely on it for termination.  Instead we scroll
 * until the item is found or we hit the physical bottom of the list.
 *
 * Returns true once the item is found (and closes the dropdown), false
 * if scrolling reaches the bottom without finding it (index doesn't
 * exist, or the list ended).
 */
export async function loadUntilIdx(page, idx, maxScrolls = 200) {
  ...
  for (let s = 0; s < maxScrolls; s++) {
    ...
    if (atBottom) {
      // We've reached the physical bottom of the list, but DevExpress
      // callbacks for the last batch may still be in-flight (especially
      // under parallel load). Wait a bit, then check one final time.
      await sleep(3_000);
      const ready = await page.evaluate((i) => {
        try {
          return cmbUtover.GetItem(i) != null;
 ...
 * data store (cmbUtover.GetItem(i)) — critical because DevExpress
 * virtual scrolling only keeps ~10 items visible in the DOM at any
 * time, so DOM-based lookups miss items in the middle of loaded batches.
 *
