...
export async function extractSplits(
  ...
  for (let vi = 0; vi < csvRaces.length; vi++) {
    ...
    const needSplits = hasPotentialSplits(race.Distanse, race.Basseng);
    ...
    try {
      ...
      // Races that can have splits need a longer wait for the callback.
      // For short races we only need Stevne — 500ms is enough.
      await sleep(needSplits ? expandDelay + jitter(jitterRange) : 500);
      ...
      // Only extract splits for races long enough to plausibly have them
      if (needSplits) {
        const splits = await page.evaluate((idx) => {
          ...
          const rows = splitTable.querySelectorAll(
            "tr.dxgvDataRow_PlasticBlue",
          );
