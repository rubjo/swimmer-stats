...
export async function extractSplits(
  ...
  for (let vi = 0; vi < csvRaces.length; vi++) {
    ...
    try {
      // Short timeout per row: if the page's JS thread is stuck (from a
      // previous callback or resource exhaustion) we bail this row quickly
      // instead of waiting for the global protocolTimeout (120s).
      await page.evaluate(
        (idx) => {
          // DevExpress may skip detail expansion for rows outside the
          // viewport. Ensure the row is visible before expanding.
        ...
        if (detailExists) {
          ...
          const diag = await page.evaluate(() => {
            return {
              ...
              inCallback: grdRanking ? grdRanking.InCallback() : "?",
            };
          });
          log(
            `    ⚠ split table not ready for row ${vi}` +
              ` (${diag.dataRows} data rows, callback: ${diag.inCallback})`,
          );
          consecutiveNotReady++;
      ...
      // Always collapse the detail row so the grid stays in a clean
      // state. Relying on DevExpress's auto-collapse (AllowOnlyOneDetailRow)
      // degrades after many expansions and causes later rows to fail.
      await page.evaluate((idx) => {
        ASPx.GVHideDetailRow("grdRanking", idx, { cancelBubble: true });
      }, vi);
