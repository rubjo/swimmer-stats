...
export async function findSwimmerIdx(page, name, opts = {}) {
  ...
  const filterIdx = await findSwimmerIdxByFilter(
    page,
    target,
    targetNorm,
    filterTerm,
  ...
  if (filterIdx !== null) return filterIdx;

  console.log(
    `  [findIdx] Filter lookup failed for "${target}" — falling back to scroll scan`,
  );
  return findSwimmerIdxByScroll(page, name, maxScrolls);
}

/**
 * Server-side-filter lookup. Types `filterTerm` into the combo, waits for the
 * DevExpress filter callback to settle, then scans the (short) filtered result
 * set for `target`. Returns the 0-based index, or null if not found.
 */
async function findSwimmerIdxByFilter(page, target, targetNorm, filterTerm, id = null) {
  // Apply the filter text and trigger DevExpress incremental filtering.
  const applied = await page.evaluate((term) => {
    try {
      cmbUtover.HideDropDown();
    } catch {}
    try {
      // SetText alone doesn't always trigger the filter callback, so we also
      // write the term into the underlying <input> and dispatch a key event,
      // which is what user typing does. Both paths funnel into the same
      // server-side incremental filter callback.
  ...
  // Wait for the filter callback to finish AND items to arrive. The filtered
  // set is small, so this settles in a few seconds — no scrolling required.
  let count = 0;
  for (let w = 0; w < 40; w++) {
    await sleep(300);
