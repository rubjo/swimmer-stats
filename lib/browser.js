      ...
      if (!detailReady) {
        ...
        if (detailExists) {
          const diag = await page.evaluate((idx) => {
            const dr = grdRanking?.GetDetailRow(idx);
            if (!dr) return { err: "no detail row" };
            const html = dr.innerHTML;
            // Find ALL tables (even without id) in the detail row
            const allTables = Array.from(dr.querySelectorAll("table")).map(
              (t) => ({
                id: t.id || "(no id)",
                rows: t.rows ? t.rows.length : "?",
                text: t.textContent.trim().slice(0, 100).replace(/\s+/g, " "),
              }),
            );
            // Search for "32,43" (the split time from user's manual check)
            const hasSplitTime = html.includes("32,43");
            // Find any element containing "32," pattern
            const splitElements = [];
            dr.querySelectorAll("*").forEach((el) => {
              const t = el.textContent.trim();
              if (t.includes("32,") || /\d+[,\.]\d+/.test(t)) {
                splitElements.push(
                  el.tagName +
                    "#" +
                    (el.id || "") +
            ...
            return {
              hasSplitTime,
              allTables,
              splitElements: splitElements.slice(0, 30),
              tailHtml: html.slice(-2000),
            };
          }, vi);
          log(
            `    - row ${vi}: hasSplitTime=${diag.hasSplitTime} tables=${JSON.stringify(diag.allTables)}`,
          );
          log(
            `    - row ${vi}: split elements: ${JSON.stringify(diag.splitElements)}`,
          );
          log(`    - row ${vi}: HTML tail (-2k):\n${diag.tailHtml}`);
          race.splits = [];
