...
export async function loadUntilIdx(page, idx, maxScrolls = 1000) {
  ...
  for (let s = 0; s < maxScrolls; s++) {
    ...
    const atBottom = await page.evaluate(async () => {
      ...
      if (d.scrollTop <= prev) {
        // scroll position didn't increase — might be at the true bottom, or
        // waiting for the server to deliver the next batch. Wait for growth.
...
export async function findSwimmerIdx(page, name, maxScrolls = 500) {
  ...
  for (let s = 0; s < maxScrolls; s++) {
    ...
    const atBottom = await page.evaluate(async () => {
      ...
      d.scrollTop = d.scrollTop + d.clientHeight;
      if (d.scrollTop <= prev) {
        // Scroll didn't increase — wait for the next batch to load
        const prevCount = cmbUtover.GetItemCount();
        for (let p = 0; p < 20; p++) {
          await new Promise((r) => setTimeout(r, 500));
          if (cmbUtover.GetItemCount() > prevCount) return false;
