...
export async function extractSplits(
  ...
  for (let vi = 0; vi < csvRaces.length; vi++) {
    ...
    try {
      ...
      await page.evaluate(
        (idx) => {
          // GVShowDetailRow is a toggle that loses internal state after
          // many calls. Click the detail expand button directly instead.
          const row = grdRanking?.GetRow(idx);
          if (!row) return;
          row.scrollIntoView({ block: "nearest" });
          const btn = row.querySelector(".dxgvDetailButton_PlasticBlue");
          if (btn) btn.click();
        },
        { timeout: 30_000 },
        vi,
      ...
      if (!detailReady) {
        ...
        if (detailExists) {
          // Collapse via click for consistency with the expand path.
          await page.evaluate((idx) => {
            const row = grdRanking?.GetRow(idx);
            if (!row) return;
            const btn = row.querySelector(".dxgvDetailButton_PlasticBlue");
