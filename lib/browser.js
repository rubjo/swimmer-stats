  ...
  for (let vi = 0; vi < csvRaces.length; vi++) {
    ...
    try {
      ...
      if (!detailReady) {
        ...
        if (detailExists) {
          // Debug: dump detail row HTML to understand why
          // the split table wasn't found by pollFor
          const detailHtml = await page.evaluate((idx) => {
            const dr = grdRanking?.GetDetailRow(idx);
            if (!dr) return "(no detail row)";
            return dr.innerHTML.slice(0, 2000);
          }, vi);
