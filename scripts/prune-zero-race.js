 ...
 *   node scripts/prune-zero-race.js --apply    # actually delete + rebuild index
 */

import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { walkJsonFiles, rebuildIndex } from "../lib/fs-utils.js";
...
for (const fp of walkJsonFiles(SWIMMERS_DIR)) {
  ...
  try {
    ...
    if (raceCount(data) === 0) zero.push({ fp, id: data.swimmerId, name: data.name });
  } catch (err) {
...
if (!APPLY) {
  console.log("\nDry run. Re-run with --apply to delete these and rebuild the index.");
  process.exit(0);
}
